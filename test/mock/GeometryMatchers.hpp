...
MATCHER_P(VectorEq, value, "") {
    ...
    return std::abs(value.real() - arg.real()) < DIST_TOLERANCE
       and std::abs(value.imag() - arg.imag()) < DIST_TOLERANCE;
}

MATCHER_P(VectorParallel, value, "") {
    constexpr double PARALLEL_TOLERANCE = 0.0625; // this gives 0.1 angle rad

    return std::abs(std::sin(std::arg(value) - std::arg(arg))) < PARALLEL_TOLERANCE;
}

MATCHER_P(VectorPerpendicular, value, "") {
    constexpr double PERPENDICULAR_TOLERANCE = 0.0625; // this gives 0.1 angle rad
